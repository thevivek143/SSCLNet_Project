...
def verify():
    ...
    if os.path.exists(brain_path):
        print(f"[OK] Brain MRI dataset found at {brain_path}")
        try:
            ds = BrainMRIDataset(brain_path)
            print(f"   Found {len(ds)} images in Brain MRI dataset.")
        except Exception as e:
            print(f"   [WARN] Could not load Brain MRI dataset: {e}")
    else:
        print(f"[FAIL] Brain MRI dataset NOT found at {brain_path}")

    if os.path.exists(knee_path):
        print(f"[OK] Knee OA dataset found at {knee_path}")
        try:
            ds = KneeOADataset(knee_path, split='train')
            print(f"   Found {len(ds)} images in Knee OA train dataset.")
        except Exception as e:
             print(f"   [WARN] Could not load Knee OA dataset: {e}")
    else:
        print(f"[FAIL] Knee OA dataset NOT found at {knee_path}")

    # 3. Model construction
    try:
        model = get_encoder()
        print("[OK] ResNet-18 Encoder constructed successfully.")
    except Exception as e:
